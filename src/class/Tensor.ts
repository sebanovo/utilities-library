import type { MethodsOfNumero, TensorDimension } from '../types/types';
  ...
  cargarSerieGeometrica(
    ...
    for (let z = 0; z < this.#layers; z++) {
      ...
      for (let x = 0; x < this.#rowLength; x++) {
        this.#tensor[z][x] ??= [];
        for (let y = 0; y < this.#columnLength; y++) {
          this.#tensor[z][x][y] = a1 * Math.round(r ** (n - 1));
          n++;
        }
      }
    }
  ...
  verificarOrdenadoRazon(r: number): boolean {
    for (let z = 0; z < this.#layers; z++) {
      for (let x = 0; x < this.#rowLength; x++) {
        for (let y = 0; y < this.#columnLength; y++) {
          if (
            y < this.#columnLength - 1 &&
            this.#tensor[z][x][y] + r !== this.#tensor[z][x][y + 1]
          ) {
            return false;
          }
          if (
            x < this.#rowLength - 1 &&
            y === this.#columnLength - 1 &&
            this.#tensor[z][x][y] + r !== this.#tensor[z][x + 1][0]
          ) {
            return false;
          }
          if (
            ...
            x === this.#rowLength - 1 &&
            y === this.#columnLength - 1 &&
            this.#tensor[z][x][y] + r !== this.#tensor[z + 1][0][0]
          ) {
            return false;
          }
        }
